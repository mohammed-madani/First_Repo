...
(function(A) {
	...
	B.formatResult = function(E, G) {
		E=E.value.toString().replace(/(?:<b.*?>)((\n|\r|.)*?)(?:<\/b>)/g, "$1");		
		var F = new RegExp("(\\" + ["/", ".", "*", "+", "?", "|", "(", ")", "[", "]", "{", "}", "\\"].join("|\\") + ")", "g"),
			H = "(" + G.toString().replace(F, "\\$1") + ")";
		return E.replace(new RegExp(H, "gi"), "<strong>$1</strong>")
	};
	B.prototype = {
		killerFn: null,
		initialize: function() {
			...
			H.fixPosition();
			if (window.opera) {
				H.el.on("keypress", function(J) {
					H.onKeyPress(J)
				})
			} else {
				H.el.on("keydown", function(J) {
					H.onKeyPress(J)
				})
			}
			H.el.on("keyup", function(J) {
				H.onKeyUp(J)
			});
			H.el.on("blur", function() {
				H.onBlur()
			});
			H.el.on("focus", function() {
				H.fixPosition()
			})
		},
		onBlur: function() {
		...
		suggest: function() {
			...
			var I = this,
				...
				F = "";
			D.each(I.suggestions, function(M, L) {
