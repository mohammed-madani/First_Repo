...
Components.utils.import("resource://quick_start.modules/last_tab.js");
Components.utils.import("resource://quick_start.modules/properties.js");
Components.utils.import("resource://quick_start.modules/restoreprefs.js");

fvd_speed_dial_FVD_SSD = function(){    
	var self = this;

	const FVD_TOOLBAR_ID = "{e9b76260-4901-4f49-8215-f88659eeb81f}";
	const SELF_ID = "faststartff@gmail.com";
	const SPEED_DIAL_URL = "chrome://quick_start/content/index.html";
	
	const HOTKEY_ELEM_ID = "fvd_speed_dial_hotKey";

    const SETTINGS_KEY_BRANCH = 'extensions.quick_start.';
    ...
    this.SPEED_DIAL_STATUSBAR_BUTTON_ID = "fvd_speed_dial_fvdSpeedDialStatusButton";
    ...
    this.status_button_insert = function(  ){
		if( self.status_button_in_toolbar(  ) ){
			return;
		}
		...
		try{
			...
			if (addonBar) {
				...
				addonBar.insertItem(self.SPEED_DIAL_STATUSBAR_BUTTON_ID, addonBarCloseButton.nextSibling);
	...
	this.status_button_remove = function(  ){		
		...
		if (addonBar) {
			...
			for each( var item in addonBar.currentSet.split(",") ){
				if( item == self.SPEED_DIAL_STATUSBAR_BUTTON_ID ){
					continue;
				}
				set.push( item );
			}
	...
	this.status_button_in_toolbar = function(){	
		return Boolean( document.getElementById( self.SPEED_DIAL_STATUSBAR_BUTTON_ID ) );
	};
    ...
    this.init = function(){     
		...
		fvd_speed_dial_LastTab.start( window );
...
//		fvd_speed_dial_RemoteAD.init( fvd_speed_dial_gFVDSSDSettings._branch() );
	...
	this.setupMainMenu = function(){
		...
		button = document.getElementById( self.SPEED_DIAL_STATUSBAR_BUTTON_ID );
